...
var (
	stdLogger    = log.New(os.Stderr, "", Ldate|Ltime|Lshortfile)
	defaultLevel = LOG_LEVEL_ALL
	logDir       = "/var/log/"
	alsoToStderr = true
)

func Config(dir string, level LogLevel, toStderr bool) {
	logDir = dir
	defaultLevel = level
	alsoToStderr = toStderr
}
...
func (l *Logger) log(t LogType, v ...interface{}) {
	...
	if !l.highlighting && alsoToStderr {
		stdLogger.Output(4, s)
	} else if l.highlighting && alsoToStderr {
...
func New(channel string) *Logger {
	logger := &Logger{
		_log:         stdLogger,
		level:        defaultLevel,
		highlighting: true,
	...
	fn := logDir + channel + ".log"
	err := logger.SetOutputByName(fn)
	if err != nil {
		log.Fatal(err)
	}
	logger.SetRotateByHour()
	logger._log.SetFlags(Ldate | Ltime | Lshortfile)
